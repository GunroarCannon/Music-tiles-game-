...
// === Scroll Panel ===
const scrollPanel = this.add.container(0, 180);
...
for (let i = 0; i < songs.length; i++) {
    const song = songs[i];
    const panel = this.createSongPanel(song.displayName, i, songs[i]);
    panel.y = y;
...
this.input.on('wheel', (_, __, ___, dy) => {
    ...
    createSongPanel(title, index, dat) {
        ...
        playBtn.on('pointerdown', () => {
            this.tweens.add({
                ...
                onComplete: () => {
                    this.scene.start('GameScene', { song: title, data : dat });
                }
            });
        });
